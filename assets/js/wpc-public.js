...
console.log('wpc_add_to_cart');
wpc_add_to_cart_multi($); return;
...
function wpc_add_to_cart_multi($) {
console.log('wpc_add_to_cart_multi');
	$('body').on('submit', 'form.cart', function (evt) {
console.log('wpc_add_to_cart_multi');
		evt.preventDefault();
		var $this = $(this);
		let productData = get_product_id_with_json_data($);
		...
		productData.forEach(function (data, index) {
			if (data.product_id) {
				simple_pro_id += ','+data.product_id;
			}
		});
		if (typeof simple_pro_id !== 'undefined' && simple_pro_id !== '') {
			form_data = form.serialize() + '&' +
				encodeURI('add-to-cart-multi') +
				'=' +
				encodeURI(simple_pro_id)
		} else {
			form_data = form.serialize();
		...
		productData.forEach(function (data, index) {
			if (data.field_name && data.field_value) {
				var paramName = data.field_name;
				form_data += '&' + encodeURI(paramName) + '=' + encodeURI(data.field_value);
			}
		});
		$.post(product_url, form_data + '&_wp_http_referer=' + product_url, function (result) {
			$(document.body).trigger('wc_fragment_refresh');
...
function get_product_id_with_json_data($) {
	let productData = [{}];
	...
	$('.wpc-addon-field:checked:not(.yith-wcqv-wrapper .wpc-addon-field)').each(function () {
		...
		var product_id = current_this.attr('data-product_id');
